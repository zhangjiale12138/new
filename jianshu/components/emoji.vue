<template>
  <div class="emoji">
    <ul class="emoji-controller">
      <li v-for="(pannel,index) in pannels"  @click="changeActive(index)" :class="{'active': index === activeIndex}" :key="index">{{ pannel }}</li>
    </ul>
...
[class*="sprite-"] {
  width: 25px;
  height: 25px;
  background: url('http://ovshyp9zv.bkt.clouddn.com/vue-emoji.png');
